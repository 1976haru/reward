#!/usr/bin/env node
/**
 * 반복 수급 탐지 룰 정책 정적 검사 (체크리스트 17 — 필수 작업 6).
 *
 * 검증:
...
const GUIDE = "docs/REPEAT_SUBSIDY_RISK_RULE.md";

const REQUIRED_FILES = [
  GUIDE,
  "src/rules/repeatSubsidyRiskRule.ts",
  "src/types/repeatSubsidyRisk.ts",
  "tests/repeatSubsidyRiskRule.test.ts",
  "scripts/run-repeat-risk-rule.ts"
];
...
async function main() {
  ...
  if (findings.length === 0) {
    console.log("CHECK_REPEAT_RISK_OK — 반복 수급 룰 모듈/가이드 존재, 필수 섹션·키워드 확인, 단정적 금지 표현 없음.");
    process.exit(0);
  }
  console.error("CHECK_REPEAT_RISK_FAIL — 반복 수급 룰 정책 위반:");
  for (const f of findings) console.error(` - ${f}`);
  process.exit(1);
}

main().catch((e) => {
  console.error("check-repeat-risk-policy.js failed:", e);
  process.exit(2);
});
