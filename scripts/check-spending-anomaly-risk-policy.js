#!/usr/bin/env node
/**
 * 예산 집행 이상 패턴 탐지 룰 정책 정적 검사 (체크리스트 20 — 필수 작업 8).
 *
 * 검증:
 ...
 *      - "금지 / 단정하지 않는다 / 확정하지 않는다 / 후보 / 확인 필요" 부정·후보 문맥에서는 허용한다.
...
const GUIDE = "docs/SPENDING_ANOMALY_RISK_RULE.md";

const REQUIRED_FILES = [
  GUIDE,
  "src/rules/spendingAnomalyRiskRule.ts",
  "src/types/spendingAnomalyRisk.ts",
  "tests/spendingAnomalyRiskRule.test.ts",
  "scripts/run-spending-anomaly-risk-rule.ts"
];
...
async function main() {
  ...
  if (findings.length === 0) {
    console.log(
      "CHECK_SPENDING_ANOMALY_RISK_OK — 예산 집행 이상 패턴 룰 모듈/가이드 존재, 필수 섹션·키워드 확인, 단정적 금지 표현 없음."
    );
    process.exit(0);
  }
  console.error("CHECK_SPENDING_ANOMALY_RISK_FAIL — 예산 집행 이상 패턴 룰 정책 위반:");
  for (const f of findings) console.error(` - ${f}`);
  process.exit(1);
}

main().catch((e) => {
  console.error("check-spending-anomaly-risk-policy.js failed:", e);
  process.exit(2);
});
