#!/usr/bin/env node
/**
 * 기관명·단체명 정규화 정책 정적 검사 (체크리스트 13 — 필수 작업 6).
 *
 * 검증:
 ...
 *      - "금지 / 단정하지 않는다 / 확정하지 않는다" 같은 부정 문맥에서는 허용한다.
...
const GUIDE = "docs/ENTITY_NORMALIZATION_GUIDE.md";

const REQUIRED_FILES = [
  GUIDE,
  "src/normalizers/entityNameNormalizer.ts",
  "src/types/entityNormalization.ts",
...
// 부정/금지 문맥 마커 — 금지 표현 뒤(또는 인접)에 등장하면 허용
const NEGATION_MARKERS = ["않", "없", "금지", "아니", "말아", "마라", "불가", "하지 않"];

function isNegatedNearby(text, phraseStart, phraseLen) {
  // 표현 앞뒤 윈도우를 검사 (뒤 25자 / 앞 12자)
...
async function main() {
  ...
  if (findings.length === 0) {
    console.log(
      "CHECK_ENTITY_NORMALIZER_OK — 정규화 모듈/가이드 존재, 필수 섹션·키워드 확인, 단정적 금지 표현 없음."
    );
    process.exit(0);
  }
  console.error("CHECK_ENTITY_NORMALIZER_FAIL — 기관명 정규화 정책 위반:");
  for (const f of findings) console.error(` - ${f}`);
  process.exit(1);
}

main().catch((e) => {
  console.error("check-entity-normalizer-policy.js failed:", e);
  process.exit(2);
});
