...
공익레이더의 표시 우선순위는 **건강기능식품 → 일반식품 → 화장품 → 의료기기 → 위조상품 → 원산지 → 보조금 → 입찰담합**입니다. 현재 1차 실전 MVP는 **건강기능식품 온라인 허위·과대광고 탐지**이며, 보조금·입찰담합은 기능을 삭제하지 않고 후순위 고급 모듈/프로토타입으로 표시합니다. 이번 재설계는 **신고분야 → 분야별 단계 워크플로우** 흐름을 메인으로 끌어올립니다.
...
- 상단 헤더의 `자동신고 없음 · 사람 검토 필수 · 수동 제출 기록만 가능` 배지는 항상 노출됩니다.
- 분야 뷰 하단의 `field-shell-note` 가 "외부 신고기관 자동 제출·자동 로그인·공식 양식 자동입력·포상금 자동신청 금지. 모든 신고 제출은 사람이 공식 창구에서 직접 진행하며, 접수번호와 결과만 수동 기록합니다." 를 고정 표시합니다.
