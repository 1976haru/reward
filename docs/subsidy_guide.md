...
> 🗂 **보조금 공개자료 소스맵**(어떤 공개자료를 어떻게 사용하는지)은 [`docs/subsidy_source_map.md`](./subsidy_source_map.md)를 참고하세요. 공공데이터포털 API 키 발급·입력 방법은 [`docs/API_SETUP.md`](./API_SETUP.md) "공공데이터포털 API 설정 가이드"에 정리되어 있습니다.
