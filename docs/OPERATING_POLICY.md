...
- **공개자료 중심 분석** — 공시·공공데이터·공개 사업 공고·공개 정산자료 만 입력으로 받는다. 비공개 자료/로그인 우회/약관 위반 수집은 [`scope.md`](../scope.md) §3 와 §4 에 의해 금지된다. 1차 조사 대상 데이터소스 정리는 [`DATA_SOURCE_MAP_GOSIMS.md`](./DATA_SOURCE_MAP_GOSIMS.md) (e나라도움 / 공개 통계센터), [`DATA_SOURCE_MAP_PUBLIC_API.md`](./DATA_SOURCE_MAP_PUBLIC_API.md) (공공데이터포털 오픈 API 후보 7건), [`LOCAL_GOV_COLLECTION_TARGETS_GYEONGGI.md`](./LOCAL_GOV_COLLECTION_TARGETS_GYEONGGI.md) (경기도 + 31개 시군 파일럿, 8개 자료 유형), [`G2B_CONTRACT_LINKAGE_MAP.md`](./G2B_CONTRACT_LINKAGE_MAP.md) (나라장터 계약데이터 5개 API 후보, 보조사업↔공공계약 동일성 후보 매핑 — 사업자/법인등록번호 원문 저장 금지, 대표자명·전화·상세주소 단독 매칭 금지) 참고 — 안보·통일 등 공개 제한 사업 / 개인 수급자 상세 / 로그인 영역 / 비공개 게시판 / 내부 행정망 / 진행 중인 감사·소송 자료는 수집 대상에서 제외된다. API 활용신청·인증키·트래픽 한도·이용허락 범위·지자체 게시판 URL 은 수집기 구현 직전 별도 확인 필수.
