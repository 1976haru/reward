...
**게시 전 광고 위반 위험을 미리 점검하는 도구.**

> Repository / internal project name: `adsafe-agent-mvp` (구 `reward-agent-mvp`)
> Product display name: 애드세이프 (AdSafe)

애드세이프(AdSafe)는 기업이 자사 광고를 게시·출시하기 **전에** 식약처·표시광고법 기준 위반 여부를 미리 점검하는 도구입니다. 사용자가 직접 입력한 광고 문구·광고 페이지 URL·업로드 파일을 분석해 위반 의심 표현을 추출하고, 종합 위험도(안전/주의/위험)와 근거·수정 제안을 담은 **광고 점검 리포트(Compliance Check Report)**를 생성합니다. 분석 엔진(extract/analyze/score/rules/dedupe/forensics)은 기존 도구의 것을 **그대로 재사용**합니다 — 광고 문구의 위반 표현을 잡는 로직은 "남의 광고를 신고하든 내 광고를 점검하든" 동일하기 때문입니다.

**안전 원칙(유지):** 애드세이프는 적법성·위반 여부를 확정하지 않습니다. "위험 신호"만 제시하며, 최종 판단은 사람이 합니다. 모든 리포트에는 "본 결과는 참고용이며, 최종 광고 적법성은 정식 자율심의·법률 자문으로 확인하십시오" 면책 푸터가 포함됩니다. 본 점검은 정식 자율심의(한국식품산업협회 등, 통상 건당 약 16.5만원)나 법적 자문을 대체하지 않습니다.

**점검 API:** `POST /api/adsafe/check` (단건) · `POST /api/adsafe/check/batch` (배치, 비용 가드) · `POST /api/adsafe/compare` (수정 후 재점검 비교) · `GET /api/adsafe/dictionary` (위반 표현 사전) · `GET /api/adsafe/info`. MOCK e2e: `npm run adsafe:mock`, 테스트: `npm run test:adsafe`.

> 참고: 아래 문서는 기존 신고 후보 탐지 흐름(보존)에 대한 설명입니다. 분석 엔진 재사용을 위해 코드·문서를 유지하며, 향후 "경쟁사 광고 벤치마킹" 등으로 재활용할 여지가 있습니다.

공익레이더(구 제품명)는 "포상금 자동화 프로그램"이 아닙니다. 사용자가 입력한 공개 URL을 수집·분석해 의심 문구를 추출하고 증거를 정리하며, 사람이 검토할 수 있는 신고서 초안을 생성합니다. 자동 신고는 수행하지 않으며, 실제 신고 제출은 사람이 공식 창구에서 직접 수행합니다. 포상금 수령을 보장하지 않습니다.
