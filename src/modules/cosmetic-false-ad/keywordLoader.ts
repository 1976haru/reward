...
const KEYWORDS_PATH = path.join(
  process.cwd(),
  "src/modules/cosmetic-false-ad/keywords.json"
);
...
function parse(raw: string): KeywordConfig {
  ...
  try {
    ...
    throw new KeywordConfigError(`invalid JSON in cosmetic keywords.json: ${(error as Error).message}`);
  }
  return validateKeywordConfig(parsed);
}

export async function loadCosmeticKeywords(): Promise<KeywordConfig> {
  if (cache) return cache;
  let raw: string;
  try {
    raw = await readFile(KEYWORDS_PATH, "utf8");
  } catch (error) {
    throw new KeywordConfigError(`failed to read cosmetic keywords.json: ${(error as Error).message}`);
  }
  cache = parse(raw);
  return cache;
}

export function loadCosmeticKeywordsSync(): KeywordConfig {
  if (cache) return cache;
  let raw: string;
  try {
...
export function clearCosmeticKeywordCache(): void {
  cache = null;
}
